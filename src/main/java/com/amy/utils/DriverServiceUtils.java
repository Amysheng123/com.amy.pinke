...
import com.amy.demo.Configure;
import com.amy.driver.beans.ChromeConfig;
import com.amy.driver.beans.GeckoConfig;
...
public class DriverServiceUtils {
    /**
     * ServiceMap用于存放浏览器名和service、options数组的键值对，map的key是浏览器名， value是object[]类型
     * 的数组，object只保存两个值，分别是driverservice和options；
     */
    ...
    /**
     * 获取指定浏览器名称的driver对象
     * 如果该浏览器对应的driver所需要的的service未启动，则读取配置文件driver_configure.yml创建service实例
     * 并根据options选项创建options对象，用service和options实例化driver.
     * 如果该浏览器对应的driver所需要连接的service已存在，则直接使用，确保service不会被反复启动
     * @param browserName 浏览器的名称，如Firefox，chrome，名称忽略大小写
    ...
    public static WebDriver getDriver(String browserName) {
        ...
        if(browserName.equalsIgnoreCase(BrowserType.CHROME)){ // 如果浏览器是谷歌浏览器
            if(!serviceMap.containsKey(BrowserType.CHROME)){ // 判断静态map中是否存在chrome字段，如果不存在，则表示该service未被创建过
                ChromeConfig config = Configure.getConfig().getChrome(); // 从配置文件中读取关于chrome的配置信息
                ...
                serviceMap.put(BrowserType.CHROME, setOptions(service, options, optionsMap)); // 组合键值对存入map中
            }
